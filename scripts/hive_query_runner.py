...
def run_beeline(cmd_args, silent=False):
    ...
    if silent: base += ["--silent=true", "--outputformat=csv2"]
...
def main():
    ...
    if not args.file.exists():
        log("R", f"FAIL File not found: {args.file}"); sys.exit(1)

    if args.dry_run:
        log("Y", f"INFO Dry run for {args.file.name}:")
        print(args.file.read_text()); return
    ...
    if ok: log("G", f"OK Completed in {time.time()-start:.1f}s")
    else: log("R", f"FAIL Execution failed"); sys.exit(3)
